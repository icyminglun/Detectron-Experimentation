...
def convert_from_cls_format(cls_boxes, cls_segms, cls_keyps):
    ...
    code.
    """
    box_list = [b for b in cls_boxes if len(b) > 0]                                 # Loops every bounding boxes in cls_boxes, (if condition) as long as there is something in cls_boxes (len(b) > 0)  [Stores looped boxes in box_list]
    if len(box_list) > 0:                                                           # If there is at least 1 bounding box contained in the box_list (len(box_list) > 0)
        boxes = np.concatenate(box_list)                                            # <np> -> from numpy library [np.concatenate] -> joins all the bounding boxes into box_list together. 
    else:
        boxes = None                                                                
...
def vis_one_image(
    ...
    if not os.path.exists(output_dir):                                                                # If path does not exists ... 
        os.makedirs(output_dir)                                                                       # This line will create a new directory called 'output_dir' 

    if isinstance(boxes, list):                                                                       # returns true if object arguement (boxes) is an instance or subclass of classinfo (list)
        boxes, segms, keypoints, classes = convert_from_cls_format(
            boxes, segms, keypoints)
    ...
    if segms is not None and len(segms) > 0:                                                          # if segms is not empty (doesn't exist) ... : 
        masks = mask_util.decode(segms)                                                               #     some pycocotools function -> takes in list type object (segms) (i think this function generally makes the segmentation into a list)

    color_list = colormap(rgb=True) / 255                                                             # <colormap> -> from utils.colormap  (instantiate colorlist?)

    kp_lines = kp_connections(dataset_keypoints)                                                      # kp_lines -> keypoints (body parts) of detection                                    
